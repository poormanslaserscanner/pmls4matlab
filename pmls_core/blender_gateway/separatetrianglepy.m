function data = separatetrianglepy( SO, H1, H2, L )
...
H1 = py2matlab(H1);
H2 = py2matlab(H2);
[H1, H2] = closeloops( H1, H2, L );
% data{1} = matlab2py( H1 );
% data{2} = matlab2py( H2 );   
% return
T1 = struct('tris', H1.tris, 'vt', H1.vt, 'selvt', H1.selvt, 'pmls_type', 'deform_mesh', 'pmls_name', [S.pmls_name, '_1']);
T2 = struct('tris', H2.tris, 'vt', H2.vt, 'selvt', H2.selvt, 'pmls_type', 'deform_mesh', 'pmls_name', [S.pmls_name, '_2']);
if isfield(SO, 'hedgehog')
    ...
    H1 = py2matlab(H1);
    H2 = py2matlab(H2);
    if hedgehogisvalid( H1 )
        T1.hedgehog = H1;
    end
    if hedgehogisvalid( H2 )
        T2.hedgehog = H2;
    end
end
if isfield(S, 'anchor')
